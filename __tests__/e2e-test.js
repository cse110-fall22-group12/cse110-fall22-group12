describe('Basic user flow for adding a new recipe', () => {
    ...
    it('On Edit Page - Check if we can add a new recipe and have contents estabalished on all pages', 
        async () => {
        ...
        await page.goto('https://cse110-fall22-group12.github.io/cse110-fall22-group12/components/view.html');
        // go to view page, check if contents are correct
        const newPageName3 = await page.title();
        expect(newPageName3).toBe('View Page');  
        const recipename = await page.evaluate(() => document.querySelector('.recipe-name').innerText);
        expect(recipename).toBe('UI test');
        const ingredient = await page.evaluate(()=> document.querySelector('#recipe-ingredients').innerText);
        expect(ingredient).toBe('UI test ingredient');
        const step = await page.evaluate(()=> document.querySelector('#recipe-preparation').innerText);
        expect(step).toBe('UI test steps');
        const notes = await page.evaluate(()=> document.querySelector('#recipe-notes').innerText);
    ...
    it('On View Page - Check if home button and edit butotn function correctly', async () => {
        ...
        newPageName4 = await page.title();
        expect(newPageName4).toBe('View Page');
        const updatedrecipename = await page.evaluate(() => document.querySelector('.recipe-name').innerText);
    ...
    it('On Edit Page - Check if the alertbox function is correct', async () => {
        ...
        expect(textvalue).toBe('Please fill out the following required fields: Recipe Name');
        // check button text 
